...
train_labels = torch.from_numpy(train_labels).to(torch.device(device),dtype=torch.long)
valid_labels = torch.from_numpy(valid_labels).to(torch.device(device),dtype=torch.long)
test_labels = torch.from_numpy(test_labels).to(torch.device(device),dtype=torch.long)
...
for epoch in range(20):
    ...
    with torch.no_grad():
        ...
        epoch_val_loss = 0
        # valid set
        idx_batches = torch.randperm(valid_N, device=device).split(batch_size)
        for idx in idx_batches:
            data = valid_x[idx, :].reshape(-1, 1,28,28)
            label = valid_labels[idx]
        #for data, label in valid_loader:
            #data = data.to(device)
            #label = label.to(device)

            val_output = att_model(data)
